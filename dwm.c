...
enum { SchemeNorm, SchemeSel, SchemeTitle, SchemeMark }; /* color schemes */
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, notitle, canfocus, issticky;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
static void autotile(Monitor* m);
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - tw - stw, 0, tw, bh, lrpad / 2, stext, 0, True);
	}
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		if (m->sel && !m->sel->notitle) {
            m->sel->issticky ? 
                drw_setscheme(drw, scheme[SchemeMark]) : 
...
focus(Client *c)
{
	...
	if (c) {
		...
		attachstack(c);
		grabbuttons(c, 1);
	    c->issticky ? XSetWindowBorder(dpy, c->win, scheme[SchemeMark][ColBorder].pixel) 
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster ? (m->ww - (gappx / 2.0)) * m->mfact : 0;
	else
		mw = m->ww;
